<a href="https://boxberry.ru/business_solutions/it_solutions/1089980/"><img align="left" width="200" src="https://lapaygroup.ru/bblogo.svg"></a>

<br /><br /><br />

# SDK для [интеграции с программным комплексом Boxberry](https://api.boxberry.ru/).

[База знаний BoxBerry](https://help.boxberry.ru/pages/viewpage.action?pageId=762955).

Данный пакет является форком. [Библиотека взятая за основу](https://github.com/iamwildtuna/boxberry-sdk).
...
```php
<?php
$bbClient = new \Grokhotov\Boxberry\Client('https://api.boxberry.ru/json.php');
...
```php
<?php
$bbClient = new \Grokhotov\Boxberry\Client();
...
try {
    $order = new \Grokhotov\Boxberry\Entity\Order();
    ...
    // $order->setVid(\Grokhotov\Boxberry\Entity\Order::PVZ); // Тип доставки (1 - ПВЗ, 2 - КД, 3 - Почта России)
    // $order->setPvzCode(61831); // Код ПВЗ
    // $order->setPointOfEntry('010'); // Код пункта поступления
    $order->setVid(\Grokhotov\Boxberry\Entity\Order::COURIER); // Тип доставки (1 - ПВЗ, 2 - КД, 3 - Почта России)
    
    $customer = new \Grokhotov\Boxberry\Entity\Customer();
    ...
    $order->setIssue(\Grokhotov\Boxberry\Entity\Order::TOI_DELIVERY_WITH_OPENING_AND_VERIFICATION); // вид выдачи (см. константы класса)

    // Для отправления Почтой России необходимо заполнить дополнительные параметры
    $russianPostParams = new \Grokhotov\Boxberry\Entity\RussianPostParams();
    $russianPostParams->setType(\Grokhotov\Boxberry\Entity\RussianPostParams::PT_POSILKA); // Тип отправления (см. константы класса)
    ...
    $russianPostParams->setPackingType(\Grokhotov\Boxberry\Entity\RussianPostParams::PACKAGE_IM_MORE_160); // Тип упаковки (см. константы класса)
...
catch (\Grokhotov\Boxberry\Exception\BoxBerryException $e) {
...
try {
    ...
    $calcParams = new \Grokhotov\Boxberry\Entity\CalculateParams();
...
**Входные параметры:**
Объект *\Grokhotov\Boxberry\Entity\Intake* - параметры заявки
...
try {
    ...
    $intake = new \Grokhotov\Boxberry\Entity\Intake();
